...
export function SupplierCard({ supplier, className }: SupplierCardProps) {
    const { addSupplier, removeSupplier, isSelected, selectedSuppliers, maxSelections } = useComparison();
    const selected = isSelected(supplier.id);
    const canAdd = selectedSuppliers.length < maxSelections;
    ...
    return (
        <div className={cn("group rounded-xl border bg-card text-card-foreground shadow-sm hover:shadow-md transition-all duration-200 flex flex-col h-full relative", className)}>
            {/* Compare Button - Positioned at top right */}
            <button
                onClick={handleCompareClick}
                className={cn(
                    "absolute top-3 right-3 z-10 flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all duration-200",
                    selected
                        ? "bg-teal-500 text-white shadow-md"
                        : canAdd
                        ? "bg-white/90 backdrop-blur-sm text-slate-600 border border-slate-200 hover:border-teal-400 hover:text-teal-600"
            ...
            <Link href={`/suppliers/${supplier.id}`} className="flex flex-col h-full">
                <div className="p-6 pt-12 space-y-4 flex-1">
                    ...
                    <div className="bg-slate-50 rounded-lg p-3 border border-slate-100">
                        <div className="flex justify-between items-start mb-1.5">
                            ...
                            <span className={cn(
                                ...
                                supplier.businessModel === "Hybrid"
                                    ? "bg-amber-50 text-amber-700 border-amber-200"
                                    : "bg-emerald-50 text-emerald-700 border-emerald-200"
                            )}>
                                {supplier.businessModel || "Circular"}
                            </span>
                        </div>
                        <p className="text-xs text-slate-700 leading-snug">
                            {supplier.capabilities?.logistics || "Logistics coverage details available in full report."}
                        </p>
                    </div>
                ...
                <div className="p-4 bg-slate-50/50 border-t grid grid-cols-2 gap-4 text-xs">
                    <div>
